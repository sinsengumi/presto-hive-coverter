...
public class HiveQueryConvertVisitor extends AstVisitor<Void, Integer> {
    ...
    public HiveQueryConvertVisitor(StringBuilder builder, Optional<List<Expression>> parameters) {
    ...
    protected Void visitExpression(Expression node, Integer indent) {
        ...
        builder.append(ExpressionConverter.formatExpression(node, parameters));
        return null;
    }
    ...
    protected Void visitUnnest(Unnest node, Integer indent) {
        builder.append("UNNEST(").append(node.getExpressions().stream()
                .map(expression -> ExpressionConverter.formatExpression(expression, parameters)).collect(joining(", ")))
                .append(")");
    ...
    protected Void visitQuery(Query node, Integer indent) {
        if (node.getWith().isPresent()) {
            ...
            while (queries.hasNext()) {
                ...
                append(indent, ExpressionConverter.formatExpression(query.getName(), parameters));
    ...
    protected Void visitQuerySpecification(QuerySpecification node, Integer indent) {
        ...
        if (node.getWhere().isPresent()) {
            append(indent, "WHERE " + ExpressionConverter.formatExpression(node.getWhere().get(), parameters))
                    .append('\n');
        }
        ...
        if (node.getGroupBy().isPresent()) {
            append(indent,
                    "GROUP BY " + (node.getGroupBy().get().isDistinct() ? " DISTINCT " : "")
                            + ExpressionConverter.formatGroupBy(node.getGroupBy().get().getGroupingElements()))
                                    .append('\n');
        }

        if (node.getHaving().isPresent()) {
            append(indent, "HAVING " + ExpressionConverter.formatExpression(node.getHaving().get(), parameters))
                    .append('\n');
        }
    ...
    protected Void visitOrderBy(OrderBy node, Integer indent) {
        append(indent, ExpressionConverter.formatOrderBy(node, parameters)).append('\n');
        return null;
    }
    ...
    protected Void visitSingleColumn(SingleColumn node, Integer indent) {
        builder.append(ExpressionConverter.formatExpression(node.getExpression(), parameters));
        if (node.getAlias().isPresent()) {
            builder.append(' ').append(ExpressionConverter.formatExpression(node.getAlias().get(), parameters));
        }
    ...
    protected Void visitJoin(Join node, Integer indent) {
        ...
        if (node.getType() != Join.Type.CROSS && node.getType() != Join.Type.IMPLICIT) {
            if (criteria instanceof JoinUsing) {
                ...
                builder.append(" ON ").append(ExpressionConverter.formatExpression(on.getExpression(), parameters));
    ...
    protected Void visitAliasedRelation(AliasedRelation node, Integer indent) {
        ...
        builder.append(' ').append(ExpressionConverter.formatExpression(node.getAlias(), parameters));
    ...
    protected Void visitSampledRelation(SampledRelation node, Integer indent) {
        ...
        builder.append(" TABLESAMPLE ").append(node.getType()).append(" (").append(node.getSamplePercentage())
                .append(')');
    ...
    protected Void visitValues(Values node, Integer indent) {
        ...
        for (Expression row : node.getRows()) {
            ...
            builder.append(ExpressionConverter.formatExpression(row, parameters));
            first = false;
        }
        builder.append('\n');
    ...
    protected Void visitShowCatalogs(ShowCatalogs node, Integer context) {
        ...
        node.getLikePattern()
                .ifPresent((value) -> builder.append(" LIKE ").append(ExpressionConverter.formatStringLiteral(value)));
    ...
    protected Void visitShowTables(ShowTables node, Integer context) {
        ...
        node.getSchema().ifPresent(value -> builder.append(" FROM ").append(formatName(value)));
    ...
    protected Void visitShowPartitions(ShowPartitions node, Integer context) {
        builder.append("SHOW PARTITIONS FROM ").append(formatName(node.getTable()));

        if (node.getWhere().isPresent()) {
            builder.append(" WHERE ").append(ExpressionConverter.formatExpression(node.getWhere().get(), parameters));
        }

        if (!node.getOrderBy().isEmpty()) {
            builder.append(" ORDER BY ").append(ExpressionConverter.formatSortItems(node.getOrderBy(), parameters));
        }
    ...
    protected Void visitDropSchema(DropSchema node, Integer context) {
        ...
        builder.append(formatName(node.getSchemaName())).append(" ").append(node.isCascade() ? "CASCADE" : "RESTRICT");
    ...
    protected Void visitRenameSchema(RenameSchema node, Integer context) {
        builder.append("ALTER SCHEMA ").append(formatName(node.getSource())).append(" RENAME TO ")
                .append(ExpressionConverter.formatExpression(node.getTarget(), parameters));
    ...
    protected Void visitCreateTableAsSelect(CreateTableAsSelect node, Integer indent) {
        ...
        if (node.getColumnAliases().isPresent()) {
            String columnList = node.getColumnAliases().get().stream()
                    .map(element -> ExpressionConverter.formatExpression(element, parameters)).collect(joining(", "));
            builder.append(format("( %s )", columnList));
        }

        if (node.getComment().isPresent()) {
            builder.append("\nCOMMENT " + ExpressionConverter.formatStringLiteral(node.getComment().get()));
        }
    ...
    protected Void visitCreateTable(CreateTable node, Integer indent) {
        ...
        String columnList = node.getElements().stream().map(element -> {
            if (element instanceof ColumnDefinition) {
                ...
                return elementIndent + ExpressionConverter.formatExpression(column.getName(), parameters) + " "
                        + column.getType()
                        + column.getComment()
                                .map(comment -> " COMMENT " + ExpressionConverter.formatStringLiteral(comment))
                                .orElse("");
            }
            if (element instanceof LikeClause) {
                LikeClause likeClause = (LikeClause) element;
                StringBuilder builder = new StringBuilder(elementIndent);
                builder.append("LIKE ").append(formatName(likeClause.getTableName()));
    ...
    private String formatProperties(List<Property> properties) {
        ...
        String propertyList = properties
                .stream().map(element -> INDENT + ExpressionConverter.formatExpression(element.getName(), parameters)
                        + " = " + ExpressionConverter.formatExpression(element.getValue(), parameters))
                .collect(joining(",\n"));
    ...
    private static String formatName(QualifiedName name) {
        return name.getOriginalParts().stream().map(HiveQueryConvertVisitor::formatName).collect(joining("."));
    }
    ...
    protected Void visitRenameTable(RenameTable node, Integer context) {
        builder.append("ALTER TABLE ").append(node.getSource()).append(" RENAME TO ").append(node.getTarget());
    ...
    protected Void visitRenameColumn(RenameColumn node, Integer context) {
        builder.append("ALTER TABLE ").append(node.getTable()).append(" RENAME COLUMN ").append(node.getSource())
    ...
    protected Void visitDropColumn(DropColumn node, Integer context) {
        builder.append("ALTER TABLE ").append(formatName(node.getTable())).append(" DROP COLUMN ")
                .append(ExpressionConverter.formatExpression(node.getColumn(), parameters));
    ...
    protected Void visitAddColumn(AddColumn node, Integer indent) {
        builder.append("ALTER TABLE ").append(node.getName()).append(" ADD COLUMN ").append(node.getColumn().getName())
                .append(" ").append(node.getColumn().getType());
    ...
    public Void visitSetSession(SetSession node, Integer context) {
        builder.append("SET SESSION ").append(node.getName()).append(" = ")
                .append(ExpressionConverter.formatExpression(node.getValue(), parameters));
    ...
    public Void visitResetSession(ResetSession node, Integer context) {
        builder.append("RESET SESSION ").append(node.getName());
    ...
    protected Void visitCallArgument(CallArgument node, Integer indent) {
        if (node.getName().isPresent()) {
            builder.append(node.getName().get()).append(" => ");
        }
        builder.append(ExpressionConverter.formatExpression(node.getValue(), parameters));
    ...
    private static void appendAliasColumns(StringBuilder builder, List<Identifier> columns) {
        if ((columns != null) && (!columns.isEmpty())) {
            String formattedColumns = columns.stream()
                    .map(name -> ExpressionConverter.formatExpression(name, Optional.empty()))
